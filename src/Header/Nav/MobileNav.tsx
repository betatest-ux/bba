...
export const MobileNav: React.FC<{ data: HeaderType; siteName: string }> = ({ data }) => {
  ...
  // Portal guard: document doesn't exist during SSR.
  ...
  return (
    <div className="lg:hidden">
      ...
      {/* Portalled to <body>: the condensed header uses backdrop-blur, and a
          backdrop-filter ancestor becomes the containing block for fixed
          descendants — inside the header this overlay would collapse to the
                ...
                <nav aria-label="Mobile menu" className="container flex-1 overflow-y-auto pb-10">
                  <ul className="space-y-1 list-none">
                    ...
                    <motion.li
                      ...
                      initial={reduceMotion ? { opacity: 0 } : { opacity: 0, y: 10 }}
                      transition={{
                        delay: reduceMotion ? 0 : 0.04 * navItems.length,
                        duration: 0.25,
                      }}
                    >
                      <Link
